...
describe('Rollback coverage guard @regression', function(){
    ...
    // stake_weight_collation_activation.js gates the utf8_bin pin on the ordering
    // that decides WHICH stake-weight rows survive the caps, on BOTH sides. A
    // drifted copy forks the stakes_root SET at an armed height (one side truncates
    // under folding order, the other under binary order) - which is exactly why the
    // pin had to be gated rather than applied to one repo at a time.
