...
const SUB = require('../../src/consensus/gates/state_subtree_gate');
...
async function teardownCrossTwin() {
    this.timeout(60000);
    sinon.restore();
...
async function assertPayloadTables() {
    this.timeout(30000);
    await stageBlocks();
...
async function assertFollowerShadows() {
    this.timeout(30000);
    await stageBlocks();
...
async function assertJournalLagIsDetectable() {
    this.timeout(30000);
...
function assertShadowMapsRestored() {
...
describe('Integration: SPV sub-tree shadow columns, cross-twin replication timing', function() {
    before(setupCrossTwin);
    after(teardownCrossTwin);
    ...
    it('the follower computes both shadow values from rows that arrived in the same block',
       assertFollowerShadows);
    it('withholding the journal rows changes the escrow shadow, so a replication lag is detectable',
       assertJournalLagIsDetectable);
    it('leaves both shadow maps exactly as it found them', assertShadowMapsRestored);
});
