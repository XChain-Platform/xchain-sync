 ...
 *
 *   text       any literal run of `xchain-sync/src/<path>` in any text file:
 *              a relative require, a comment, a shell script, a markdown runbook.
 *   join       a path built segment by segment, the shape
 *              path.join(root, 'xchain-sync', 'src', 'foo.js'). When every
 ...
 *              `const R = path.resolve(__dirname, '../../xchain-sync')` then
 *              path.join(R, 'src', 'foo.js'), path.join(R, 'src/foo.js') or
 *              `${R}/src/foo.js`. The root may equally be
 *              process.env.XCHAIN_SYNC_PATH, a candidate array filtered to its
 ...
 *              this checkout, `syncFile('src/rollback.js')`. A call over a
 *              literal array, `syncFile('src/' + twin)`, is emitted once per
 ...
 *              `copy_twin xchain-sync "src/$f"`, resolved against the literal
...
// `xchain-sync/src/<path>`, however it was spelled: a relative require
// (`../../xchain-sync/src/utility.js`), a prose mention, a shell path.
const TEXT_REFERENCE = new RegExp(`${NAME_RE}\\/(src\\/[A-Za-z0-9_@.\\-/]+)`, 'g');

// path.join(..., 'xchain-sync', 'src', ...): the tail is captured raw and
...
// Environment variables that hold a checkout of this repo. XCHAIN_SYNC_SQL_PATH
...
const ENV_ROOT_VARS = ['XCHAIN_SYNC_PATH', 'XCHAIN_SYNC_DIR'];
...
function rootPrefix(init) {
    ...
    while ((m = re.exec(init)) !== null) {
        ...
        // path.join(base, 'xchain-sync', 'src', 'sql').
...
function collectRootProducers(text) {
    ...
    while ((m = re.exec(text)) !== null) {
        ...
        // `const p = syncFile('src/x.js')` look like another checkout.
 ...
 * checkout, the `syncFile('src/rollback.js')` idiom. Recognised by a body
...
function helperReferences(text, helpers, lists, strings) {
    ...
    for (const [name, suffix] of helpers) {
        ...
        let m;
        while ((m = re.exec(text)) !== null) {
            // `function syncFile(rel){` reads as a call of itself. Its
 ...
 * relative path, `copy_twin xchain-sync "src/$f"`, where the file comes from
