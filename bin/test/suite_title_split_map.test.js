...
const { compare, applyNameMap } = require('../suite-title-map.js');
...
describe('bin/suite_title_map/split_map.js: a declared split that is honest', () => {
    ...
    it('rewrites pinned titles through the declared name map before grading the union', () => {
        const pin = { test: { [OLD]: ['db orders _readOrders'].concat(OLD_TITLES.slice(1)), [OTHER]: ['other one'] } };
        const tree = JSON.parse(JSON.stringify(FRESH));
        tree.test[ORDERS] = ['db orders readOrders', 'db orders writes'];
        const graded = (names) => splitMap.compareWithSplits({
            pin: mapOf(pin), fresh: mapOf(tree), renames: {}, splits: SPLIT,
            compare: (p, f, r, o) => compare(p, f, r, o, names),
            retitle: (t) => applyNameMap(t, names),
        });
        assert.deepStrictEqual(graded({ _readOrders: 'readOrders' }), []);
        // The same symbol rename left out of the name map is still a finding.
        assert.deepStrictEqual(kinds(graded({})), ['title_added', 'title_dropped']);
    });
});
