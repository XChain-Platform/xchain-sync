 ...
 * the sibling. Several modules in this tree are exactly that shape. A file with no
 ...
 * shape the sweep deletes outright.
 *
...
const DYNAMIC_EDGES = [
    {
        // Inert until the db split lands, because an edge only applies to a file
        // that exists: src/db.js requires nothing by computed path today. It is
        // declared ahead of the split rather than after it so the split cannot
        // silently make every mixin read unreachable.
...
function twinCopies(sources, siblingsRoot) {
    ...
    try {
        repos = fs.readdirSync(siblingsRoot, { withFileTypes: true })
            .filter((e) => e.name.startsWith('xchain-') && e.name !== 'xchain-sync')
            .map((e) => e.name)
            .sort();
    } catch (e) {
        return out;
...
function main() {
    ...
    console.log(`reachable from service runtime:      ${s.reachableFromRuntime}`);
    console.log(`NOT reachable from service runtime:  ${s.notReachableFromRuntime}`);
