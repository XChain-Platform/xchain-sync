 ...
 *
 * TITLES ARE PORTABLE, WHICH IS NOT FREE. Some suites build a title from a
 * resolved path (the twin-parity guards name the sibling file they compare), so
 * a title captured verbatim carries the absolute location of whoever ran it.
 * Two things break on that: the pin stops reproducing at any other checkout,
 * which is the one property a pin exists to have, and a tracked file gains an
 * operator path, which the platform's public-words rule forbids. Every absolute
 * path inside this checkout or the tree around it is therefore rewritten to a
 * stable token before the title is hashed, so the same tree pins the same bytes
 * wherever it sits.
 *
 * USAGE
...
const SIBLINGS_ROOT = path.resolve(REPO_ROOT, '..');

/**
 * A title with every checkout-dependent absolute path replaced by a token.
 * The repo root is tried before the siblings root because it is the longer
 * prefix of the two and a shorter match first would leave `/xchain-sync` glued
 * to the token.
 *
 * @param {string} title a full test title as mocha reported it
 * @returns {string} the same title, reproducible from any checkout location
 */
function portableTitle(title) {
    return title.split(REPO_ROOT).join('<repo>').split(SIBLINGS_ROOT).join('<siblings>');
}
...
function collect(scriptName, script) {
    ...
    for (const test of (report.tests || []).concat(report.pending || [])) {
        ...
        files[rel].push(portableTitle(test.fullTitle));
    }
    const sorted = {};
    let titles = 0;
