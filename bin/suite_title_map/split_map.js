 ...
 * has in the tree just before it is split. The name map applies first too: the
 * CLI passes `retitle`, the same symbol rewrite compare() makes inside every
 * pinned title, so a split can land beside a declared symbol rename and the
 * union is graded against the titles as renamed.
 *
...
/**
 * A script's pinned files under their renamed paths, each title passed through
 * `retitle`, and which pin keys each came from.
 */
function renamedView(before, renames, retitle) {
    const mapped = {};
    const origin = {};
    for (const rel of Object.keys(before)) {
        const name = renames[rel] || rel;
        mapped[name] = before[rel].map((t) => retitle(t));
 ...
 * `compare` is called with four arguments, so a declared name map travels bound
 * into it; `retitle` is that name map's rewrite of one pinned title.
 */
function compareWithSplits({ pin, fresh, renames, splits, only, compare, retitle = (t) => t }) {
    ...
    for (const name of Object.keys(pin.scripts).filter((n) => !only || n === only).sort()) {
        ...
        const { mapped, origin } = renamedView(before, renames, retitle);
