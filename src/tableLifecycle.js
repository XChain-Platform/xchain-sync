...
const TABLES = [
    ...
    { table: 'tokens', owner: 'indexer', replication: 'stream:action', rollback: 'action', replicaRollback: 'mirror', alsoRecomputed: true,
      hashed: { classes: [],
                note: 'Action-derived; supply is additionally recomputed and sanity-checked against credits/debits/escrows each block. KNOWN GAP (F-1, pinned in the sync coverage suite): the in-place supply mutation on a surviving token row rides no updated_rows class and no state_hash class.' } },
    ...
    { table: 'polls', owner: 'indexer', replication: 'stream:action', rollback: 'action', replicaRollback: 'mirror',
      hashed: { classes: ['state_hash'],
                note: 'The in-place finalization flip on a surviving polls row (updated_rows POLL_FINALIZE channel) is covered by the state_hash poll_finalize class, flag-day gated per chain (POLL_FINALIZE_STATE_HASH_ACTIVATION, inert 999999999 until armed with the other flag-day gates). New rows are otherwise action-derived.' } },
